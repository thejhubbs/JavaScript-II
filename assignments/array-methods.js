...
let fullName = [];
fullName = runners.map(function(runner) { return runner["first_name"] + " " + runner["last_name"];  });
...
ticketPriceTotal = runners.reduce(function(total, runner) { return total + runner["donation"]; }, 0);
...
// Problem 1
//Alphabetically sort company name list
let companyNames = runners.map(function(runner) { return runner["company_name"]; }).sort(); 
console.log(companyNames);

// Problem 2
//List the emails of the runners with L shirt sizes
let largeShirtEmails = largeShirts.map(function(runner) { return runner["email"]; });
console.log(largeShirtEmails);

// Problem 3
//People who donated 2x more than average
let average = ticketPriceTotal / runners.length;
let bigDonors = runners
                .filter(function(runner) { return runner["donation"] > average*2; })
                .map(function(runner) { return runner["first_name"]});
